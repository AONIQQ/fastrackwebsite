...
export const FIXTURE_RESULT_QUARANTINE_SCOPE = 'fixture_result_quarantine'
...
export function fixtureResultQuarantineRolloutReady(snapshot, env = process.env) {
  return fixtureResultDispatchEnabled(env)
    && snapshot?.configurationStatus === 'valid'
    && snapshot.configured.resultsEnqueue === false
    && snapshot.configured.resultsDispatch === false
...
export function fixtureResultQuarantineResponse(status) {
  if (status === 'quarantined') return Object.freeze({ ok: true, status: 'fixture_result_quarantined' })
  if (status === 'stopped') return Object.freeze({ ok: false, status: 'fixture_result_quarantine_stopped' })
  if (status === 'failed') return Object.freeze({ ok: false, status: 'fixture_result_quarantine_failed' })
  return Object.freeze({ ok: false, status: 'fixture_result_quarantine_blocked' })
}
