...
export function normalizeWhopEvent(event) {
  if (!event || event.api_version !== 'v1' || !WHOP_WEBHOOK_EVENTS.includes(event.type) || !IDENTIFIER.test(String(event.id || ''))) return null
  ...
  if (!data) return null
  const payment = event.type.startsWith('payment.') ? data : data.payment
  ...
  // Whop's refund.updated and dispute.updated v1 schemas do not include an
  ...
  const amountCents = event.type.startsWith('refund.') || event.type.startsWith('dispute.') ? cents(data.amount) : paymentAmountCents
  const currencyValue = data.currency ?? payment.currency
  return {
    eventId: event.id, eventType: event.type, objectId, paymentId,
    ...
    state: event.type.startsWith('refund.') ? String(data.status || 'unknown').toLowerCase().slice(0, 64)
      : event.type.startsWith('dispute.') ? disputeState(data.status)
        : event.type === 'payment.succeeded' ? 'paid' : 'failed',
...
function decodeSigningKey(secret) {
  ...
  if (key.length < 24 || key.length > 64 || key.toString('base64') !== encoded) return null
  return key
}
