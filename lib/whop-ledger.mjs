...
export function normalizeWhopEvent(event) {
  ...
  if (!data) return null
  const payment = event.type.startsWith('payment_') ? data : data.payment
  ...
  const amountCents = event.type.startsWith('refund_') || event.type.startsWith('dispute_') ? cents(data.amount) : paymentAmountCents
  ...
  return {
    ...
    state: event.type.startsWith('refund_') ? String(data.status || 'unknown').toLowerCase().slice(0, 64)
      : event.type.startsWith('dispute_') ? disputeState(data.status)
