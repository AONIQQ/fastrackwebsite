...
export async function ingestResendWebhook({
  rawBody, headers, secret, verify, persist, nowSeconds = undefined,
  ingestionEnabled = false, projectionEnabled = false,
}) {
  // A stopped endpoint deliberately returns a retryable response. Signed events
  // are not acknowledged unless they have been durably recorded.
  if (!ingestionEnabled) return { status: 503, body: { error: 'Webhook ingestion stopped' } }
  ...
  const result = await persist(event, { project: projectionEnabled })
  ...
  if (result.duplicate) return { status: 200, body: { ok: true, duplicate: true, projected: Boolean(result.projected) } }
  return { status: 200, body: { ok: true, outcome: result.outcome, projected: Boolean(result.projected) } }
}
