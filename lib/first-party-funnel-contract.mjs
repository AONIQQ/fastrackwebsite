...
export const FIRST_PARTY_FUNNEL_SOURCES = Object.freeze([
  'direct', 'reddit', 'facebook', 'instagram', 'tiktok', 'forum', 'email', 'youtube', 'google', 'bing', 'referral', 'podcast',
])
...
const PARTNER_PLACEMENT_CONTENT = /^partner-p[0-9]{4}$/
const ALEXIS_CREATOR_CONTENT = /^alexis-v[0-9]{3}$/
const UUID_V4 = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/
const CAMPAIGN = /^(?:agent-\d{8}|creator-\d{8}|qa-[a-z0-9](?:[a-z0-9-]{0,46}[a-z0-9])?|validation|direct)$/
...
export function normalizeFirstPartyAttribution(input = {}) {
  ...
  const placementContent = normalizedContent !== null
    && PARTNER_PLACEMENT_CONTENT.test(normalizedContent)
    && source !== 'direct'
    && ['partner', 'referral'].includes(medium)
  const creatorContent = normalizedContent !== null
    && ALEXIS_CREATOR_CONTENT.test(normalizedContent)
    && ['instagram', 'tiktok', 'facebook', 'youtube'].includes(source)
    && medium === 'organic'
    && campaign.startsWith('creator-')
  const content = normalizedContent !== null && (contentSet.has(normalizedContent) || placementContent || creatorContent)
    ? normalizedContent
    : null
