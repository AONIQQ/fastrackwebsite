...
/**
 * Produce the bounded server-log diagnostic independently of whether the
 * caller is authorized to receive that diagnostic in the HTTP response.
 * Ordinary public failures must remain opaque to the visitor while retaining
 * the same privacy-safe operational signal in server logs.
 *
 * @param {'risk_claim'|'college_lookup'|'roi_compute'|'lead_insert'|null} phase
 * @param {unknown} error
 */
export function captureFailureLogDiagnostic(phase, error) {
  return phase !== null && PHASES.has(phase)
    ? captureFailureDiagnostic(phase, error)
    : null
}
