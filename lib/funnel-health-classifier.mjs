export const FUNNEL_HEALTH_THRESHOLDS = Object.freeze({
  ...
  nurtureEnqueueWarningHours: 18,
  nurtureEnqueueCriticalHours: 24,
...
export function classifyFunnelHealth(input, now = new Date()) {
  ...
  if (input.missingNurtureRows > 0 && input.missingNurtureOldestHours !== null) {
    components.nurture_enqueue = input.missingNurtureOldestHours >= FUNNEL_HEALTH_THRESHOLDS.nurtureEnqueueCriticalHours
      ? 'CRITICAL'
      : input.missingNurtureOldestHours >= FUNNEL_HEALTH_THRESHOLDS.nurtureEnqueueWarningHours ? 'WARNING' : 'READY'
  }
