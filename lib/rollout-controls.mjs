...
export function publicRolloutStatus(env = process.env) {
  ...
  return Object.freeze({
    controls: Object.freeze(Object.fromEntries(
      Object.entries(status).map(([key, value]) => [key, Object.freeze({
        ...value,
        effective: effective[key],
      })]),
    )),
    dependency_status: rolloutDependencyWarnings(configured).length ? 'invalid' : 'valid',
    dependency_warnings: rolloutDependencyWarnings(configured),
  })
}
