...
export function captureAcknowledgementReady(controls) {
  return controls.captureAcknowledgement && controls.shadowLedger && controls.resultsEnqueue
}

export function canClaimMessage(kind, status, controls) {
  if (kind === 'results') {
    if (!controls.resultsDispatch) return false
    return status === 'pending' || ((status === 'retryable' || status === 'claimed') && controls.resultsRetry)
  }
  if (kind === 'nurture') {
    if (!controls.nurtureClaim || !controls.nurtureDispatch) return false
