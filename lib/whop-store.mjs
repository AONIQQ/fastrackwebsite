...
with refund_per_object as (
  ...
  where provider='whop' and provider_payment_id=$1 and event_type in('refund_created','refund_updated')
  order by object_id,lifecycle_at desc,
    case when event_type='refund_updated' then 1 else 0 end desc,event_id desc
), refunds as (
  ...
  from payment_provider_events
  where provider='whop' and provider_payment_id=$1 and event_type in('dispute_created','dispute_updated')
  order by object_id,lifecycle_at desc,
    case when event_type='dispute_updated' then 1 else 0 end desc,event_id desc
), dispute as (
  select case
...
where provider='whop' and provider_payment_id in(select provider_payment_id from projected)
  and event_type in('refund_created','refund_updated','dispute_created','dispute_updated')`
...
export async function persistWhopEvent(db, event, context) {
  ...
  if (event.eventType === 'payment_succeeded') {
    ...
    await db.query(WHOP_EVENT_SQL, [
      ...
      event.providerCreatedAt,event.lifecycleAt,event.eventType === 'payment_failed' ? 'ignored' : 'received',fixture,
    ])
  }
