...
export function createSlidingWindowLimiter({ limit = 8, windowMs = 60_000, maxKeys = 5_000 } = {}) {
  ...
  return (key, now = Date.now()) => {
    ...
    if (attempts.size > maxKeys) {
      for (const [candidate, times] of attempts) {
        if (times.every((time) => time <= cutoff)) attempts.delete(candidate)
...
export function captureResponseIsAcknowledged(value, captureId) {
  return Boolean(value && value.ok === true && Number.isSafeInteger(value.id) && value.id > 0 && value.capture_id === captureId && value.roi && typeof value.roi === 'object')
}
