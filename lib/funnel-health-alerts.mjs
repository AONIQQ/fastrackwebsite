...
export function actionableFunnelIssues(report) {
  ...
  const nurtureEligibility = report.nurture_eligibility ?? {}
  const missingNurture = Number(nurtureEligibility.missing_due ?? 0)
  const missingNurtureAge = nurtureEligibility.oldest_due_at
    ? Math.max(0, (Date.parse(report.generated_at) - Date.parse(nurtureEligibility.oldest_due_at)) / 3_600_000)
    : null
  const nurtureEnqueueLevel = report.component_status?.nurture_enqueue
  if (missingNurture > 0 && missingNurtureAge != null && (nurtureEnqueueLevel === 'WARNING' || nurtureEnqueueLevel === 'CRITICAL')) {
    issues.push(issue('NURTURE_ENQUEUE_MISSING', nurtureEnqueueLevel, missingNurture, { age_hours: missingNurtureAge }))
  }
...
const LABELS = Object.freeze({
  ...
  NURTURE_ENQUEUE_MISSING: 'Nurture-eligible lead missing message row',
