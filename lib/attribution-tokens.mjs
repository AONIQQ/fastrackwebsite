...
export function createUniqueCheckoutToken({ trackingId, step, expiresAt, nonce }, secret, nowSeconds = Math.floor(Date.now() / 1000)) {
  if (!validateClaims({ trackingId, step, expiresAt }, nowSeconds) || !UUID_V4.test(String(nonce || ''))) {
    throw new Error('invalid checkout token claims')
  }
  const unsigned = `v2.x.${trackingId}.${step}.${expiresAt}.${nonce}`
  return `${unsigned}.${sign(unsigned, secret)}`
}

export function verifyCheckoutToken(token, secret, nowSeconds = Math.floor(Date.now() / 1000)) {
  const parts = String(token || '').split('.')
  const v1 = parts.length === 6 && parts[0] === 'v1' && parts[1] === 'x'
  const v2 = parts.length === 7 && parts[0] === 'v2' && parts[1] === 'x' && UUID_V4.test(parts[5])
  if (!v1 && !v2) return null
  const [, , trackingId, step, expiry] = parts
  const signature = parts.at(-1)
  const expiresAt = Number(expiry)
  if (!validateClaims({ trackingId, step, expiresAt }, nowSeconds)) return null
  const unsigned = parts.slice(0, -1).join('.')
  return safeSignatureEqual(signature, sign(unsigned, secret))
    ? { trackingId, step, expiresAt, ...(v2 ? { nonce: parts[5] } : {}) }
    : null
}
