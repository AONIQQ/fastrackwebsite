        ...
        <div key={window} className="mb-5 last:mb-0">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr><th className="text-left p-1">Class</th><th className="text-left p-1">Source / campaign</th><th className="text-right p-1">Intent</th><th className="text-right p-1">Modal</th><th className="text-right p-1">Attempt</th><th className="text-right p-1">Captured</th><th className="text-right p-1">Failed</th><th className="text-right p-1">Captured / intent</th><th className="text-right p-1">Captured / attempt</th></tr></thead>
              <tbody>
                {rows.filter((row) => row.window === window).map((row) => <tr key={`${row.traffic_class}-${row.source}-${row.medium}-${row.campaign}`} className="border-t border-[#e0e0f0]"><td className="p-1 font-bold">{row.traffic_class}</td><td className="p-1">{row.source} / {row.medium} / {row.campaign}</td><td className="p-1 text-right">{row.intent}</td><td className="p-1 text-right">{row.modal_opened}</td><td className="p-1 text-right">{row.submission_attempted}</td><td className="p-1 text-right">{row.lead_captured}</td><td className="p-1 text-right">{row.capture_failed}</td><td className="p-1 text-right">{ratio(row.captured_per_intent)}</td><td className="p-1 text-right">{ratio(row.captured_per_attempt)}</td></tr>)}
