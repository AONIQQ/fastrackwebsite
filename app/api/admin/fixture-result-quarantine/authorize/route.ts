...
export async function POST(request: Request) {
  ...
  if (!isAllowedCaptureOrigin(request.headers.get('origin'), request.url) || !isAdmin()) {
    return NextResponse.json(fixtureResultQuarantineResponse('blocked'), { status: 401, headers })
  }
  try {
    ...
    const authorization = createScopedFixtureAuthorization(
      process.env.ADMIN_TOKEN, FIXTURE_RESULT_QUARANTINE_SCOPE, input.captureId,
    )
    return NextResponse.json({ authorization }, { headers })
  } catch {
    return NextResponse.json(fixtureResultQuarantineResponse('blocked'), { status: 400, headers })
  }
}
