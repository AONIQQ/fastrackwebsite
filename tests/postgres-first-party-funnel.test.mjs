...
test('PostgreSQL 17 freezes attribution, deduplicates concurrent stages, bounds ingest, and reports QA separately', {
  ...
  psql(`insert into calculator_funnel_sessions(session_digest,utm_source,utm_medium,utm_campaign,utm_content,traffic_class) values('${'5'.repeat(64)}','podcast','partner','agent-20260820','partner-p0001','business'); insert into calculator_funnel_events(session_digest,event_name) values('${'5'.repeat(64)}','Calculator Intent')`)
  ...
  assert.equal(psql(`begin; insert into calculator_funnel_sessions(session_digest,utm_source,utm_medium,utm_campaign,utm_content,traffic_class) values('${'e'.repeat(63)}1','youtube','organic','creator-20260820','alexis-v012','business'); select utm_source||'|'||utm_medium||'|'||utm_campaign||'|'||utm_content from calculator_funnel_sessions where session_digest='${'e'.repeat(63)}1'; rollback;`), 'youtube|organic|creator-20260820|alexis-v012')
