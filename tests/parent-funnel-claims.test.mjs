...
test('calculator labels its scenario and surfaces assumptions before capture', async () => {
  ...
  assert.match(source, /Transfer, degree fit, residency, course availability, aid, and timing vary/)
  assert.match(source, /net price is an average for federal-aid recipients, not your family&?rsquo;s personalized aid/i)
  assert.match(source, /Earnings figures use two years of College Scorecard median post-enrollment earnings/)
  assert.match(source, /not a\s+wage forecast/)
